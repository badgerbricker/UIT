...
parser.add_argument(
    '-r', '--samplerate', type=float, default=20000, help='sampling rate of audio device')#so long as 2*max_frequency should be good
parser.add_argument(
...
parser.add_argument('-rr', '--range', type=float, nargs=2,
                    metavar=('LOW', 'HIGH'), default=[300, 3000],
                    help='frequency range (default %(default)s Hz)')#human voice range is 300-4k
parser.add_argument(
...
xf = np.linspace(args.range[0], args.range[1], CHUNK)
...
##########################filtering
'''
frq=200#frequenct range
off_set=100 #frequency offset
t=np.zeros(CHUNK)
filter=(1/(np.pi*t))*np.sin(frq*t) 
'''
...
global plotdata2
yf=np.zeros(((args.range[1]-args.range[0])))
...
try:
    ...
    ax2.set_xlim(args.range[0], args.range[1])
    plt.setp(ax2, xticks=[0,100,args.range[1]],yticks=[0, 0.5,1])
